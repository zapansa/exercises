...
TEST(FoodWastageTrackerBackend, DeleteRecordFromEmptyTracker) {
  FoodWastageTrackerBackend backend{""};
  json delete_status =
      json::parse(backend.DeleteRecord(CreateDefaultQueryString()).dump());
...
TEST(FoodWastageTrackerBackend, DeleteNonexistentRecordFromTracker) {
  FoodWastageTrackerBackend backend{""};
...
TEST(FoodWastageTrackerBackend, AccidentalDoubleDeletion) {
  FoodWastageTrackerBackend backend{""};
...
TEST(FoodWastageTrackerBackend, AddDeleteGetRecord) {
...
TEST(FoodWastageTrackerBackend, GetReportWasteReductionWithDeleteTest) {
  FoodWastageTrackerBackend backend{""};
  ...
  json delete_status =
      json::parse(backend
                      .DeleteRecord(CreateDefaultQueryStringWithWastageReason(
                          kTooMuchLeftOverT))
                      .dump());
  ASSERT_TRUE(delete_status.at("success"))
      << "Failed to delete the food wastage record.";
  ...
  // Waste reduction strategies
  strategiesJson = report.at("suggested_strategies_to_reduce_waste_");
  ASSERT_TRUE(jsonContains(strategiesJson, kBuyLessFoodT))
      << "Food wastage report should suggest buying less food.";
  ASSERT_TRUE(jsonContains(strategiesJson, kRecycleLeftOversT))
      << "Food wastage report should suggest recycling lefotvers.";
  ...
  delete_status = json::parse(
      backend
          .DeleteRecord(CreateDefaultQueryStringWithWastageReason(kTastesBadT))
          .dump());
  ASSERT_TRUE(delete_status.at("success"))
  ...
  // Delete record
  delete_status = json::parse(
      backend.DeleteRecord(CreateDefaultQueryStringWithWastageReason(kExpiredT))
          .dump());
  ASSERT_TRUE(delete_status.at("success"))
...
TEST(FoodWastageTrackerBackend, AddAndDeleteManyRecords) {
  FoodWastageTrackerBackend backend{""};
